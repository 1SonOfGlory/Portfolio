import {publicationFeed} from './publication-date.mjs';
...
export async function feed(){if(staticProduction)return [];if(!cloud)return publicationFeed(await local('feed'));const {data,error}=await supabase.rpc('journal_feed');if(error)throw error;return publicationFeed(data||[]);}
